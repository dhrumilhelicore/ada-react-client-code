...
const Depression = () => {
  ...
  document.title = "Depression | Ada";

  useEffect(() => {
    getDepressionPageDetalis();
    window.scroll(0, 0);
  }, []);
  ...
  const getDepressionPageDetalis = () => {
    getInnerPageDetails(STATICTODYNAMIC, "mental_health", "Depression").then(
      (response) => {
        const {
        ...
        setMetaData(meta_data.find((item) => item.page_name === "depression"));
        setGlobalSetting(
