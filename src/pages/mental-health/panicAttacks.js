...
const PanicAttacks = () => {
  ...
  document.title = "Panik Attackes | Ada";
  ...
  const getPanicPageDetalis = () => {
    getInnerPageDetails(STATICTODYNAMIC, "mental_health", "Panic Attacks").then(
      (response) => {
        const {
        ...
        setMetaData(meta_data.find((item) => item.page_name === "panic-attacks"));
        setGlobalSetting(
