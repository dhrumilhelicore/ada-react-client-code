...
const MoodDisorder = () => {
  ...
  document.title = "Mood Disorder | Ada";

  useEffect(() => {
    getMoodDisorderPageDetalis();
    window.scroll(0, 0);
  }, []);
  ...
  const getMoodDisorderPageDetalis = () => {
    getInnerPageDetails(STATICTODYNAMIC, "mental_health", "Mood Disorder").then(
      (response) => {
        const {
        ...
        setMetaData(meta_data.find((item) => item.page_name === "mood-disorder"));
        setGlobalSetting(
