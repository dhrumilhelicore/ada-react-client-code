...
const PostTraumatic = () => {
  ...
  document.title = "Post Traumatic Stress Disorder | Ada";

  useEffect(() => {
    getPostTraumaticPageDetalis();
    window.scroll(0, 0);
  }, []);
  ...
  const getPostTraumaticPageDetalis = () => {
    getInnerPageDetails(
      STATICTODYNAMIC,
      "mental_health",
      ...
      setMetaData(meta_data.find((item) => item.page_name === "post-traumatic-stress-disorder"));
      setGlobalSetting(
