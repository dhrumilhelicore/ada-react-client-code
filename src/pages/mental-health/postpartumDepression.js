...
const PostPartumDepression = () => {
  ...
  document.title = "Postpartum Depressions | Ada";

  useEffect(() => {
    getPostPartumPageDetalis();
    window.scroll(0, 0);
  }, []);
  ...
  const getPostPartumPageDetalis = () => {
    getInnerPageDetails(
      STATICTODYNAMIC,
      "mental_health",
      ...
      setMetaData(meta_data.find((item) => item.page_name === "postpartum-depression"));
      setGlobalSetting(
